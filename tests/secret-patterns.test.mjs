...
import { allowlistReason, scanRepository, scanText } from "../scripts/scan-secret-patterns.mjs";
...
test("scanner recognizes each supported high-confidence credential shape", () => {
  const candidates = [
    ...
    ["Cloudflare API token assignment", ["CF_API_", "TOKEN=", "a".repeat(40)].join("")],
  ];
...
test("allowlist is limited to generated content, lockfiles, licenses, and fake fixtures", () => {
  ...
  assert.equal(allowlistReason(fakeFixture), "isolated, intentionally fake secret-scanner fixture");
  assert.equal(
