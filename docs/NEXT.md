...
1. Add a static-export check that every HTML document has exactly one `head` element.
2. Add a static-export check that every `base` URL, if present, stays on the Store Canary origin.
3. Add a static-export check that an `html` element's `xmlns`, when present, is the HTML namespace
...
27. Add a static-export check that no element retains `aria-invalid="true"`.
28. Add a static-export check that every `nav` has a non-empty accessible name when a document has
    more than one.
29. Add a static-export check that the document body does not have a `hidden` attribute.
30. Add a static-export check that `theme-color` and `color-scheme` metadata, when present, each has
    non-empty content.
31. Add a static-export check that no static disclosure retains `aria-expanded="true"`.
32. Add a static-export check that every `label` with a `for` attribute references an existing
    control in the same document.
33. Add a static-export check that the document body does not have `spellcheck="false"`.
